...
def compute_silhouette(data, labels):
    ...
    for i in range(n):
        ...
        if len(same_cluster) <= 1:
            ...
            a = np.mean(dist_matrix[i, same_cluster[same_cluster != i]])
        ...
        for c in unique_labels:
            ...
            dist = np.mean(dist_matrix[i, cluster_points])

            if dist < b:
                b = dist

        denom = max(a, b)
        sil[i] = 0 if denom == 0 else (b - a) / denom

    return sil, np.mean(sil)
...
class CSDPPO_UI:
    ...
    def cluster(self, event):
        ...
        sil_scores, sil_mean = compute_silhouette(self.data, labels)

        print(f"Silhouette: {sil_mean:.4f}")
...
if __name__ == "__main__":
    ...
    data = load_data("Acute1_4_Nhom3Input.txt")

    dist_matrix = compute_distance_matrix(data)

    dist_list = get_distance_list(dist_matrix)

    dc = find_optimal_dc(dist_list, dist_matrix, 3, 10)
    ...
    delta, nearest_higher, comparisons = compute_delta_pgd(data,dist_matrix, rho, dc, k=2.0)
